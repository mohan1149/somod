	...
	<table class="table table-slim no-border">
		@foreach ($transactions as $transaction)
			<tr class="cursor-pointer" 
				...
				<td>
				@if($transaction->table){{ $transaction->table->name }} - @endif {{ $transaction->invoice_no }} ({{optional($transaction->contact)->name}})
				</td>
				...
				<td>
	    			...
	    			<a href="{{action('SellPosController@destroy', [$transaction->id])}}" class="oex-delete-sale" data-id="{{$transaction->id}}" style="padding-left: 20px; padding-right: 20px"><i class="fa fa-trash text-danger" title="{{__('lang_v1.click_to_delete')}}"></i></a>
