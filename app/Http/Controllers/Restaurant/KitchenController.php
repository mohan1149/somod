...
class KitchenController extends Controller
{
    ...
    public function markAsCooked($id)
    {
	 ...
	 public function markAsCookedItem(Request $request)
    {
        // if (!auth()->user()->can('sell.update')) {
        //     abort(403, 'Unauthorized action.');
        // }
        try {
			...
			$transatcion_id=$request->get('transaction_id');
            ...
            $transaction = Transaction::where('business_id', $business_id)
                            ...
                            ->find($transatcion_id);
            if (!empty($transaction)) {
				$cooking_status= array_map('intval', explode(',', $transaction->OEX_KITCHEN_STATUS ));
				
                if(!in_array($item_id,$cooking_status))
				{ 
					
					array_push($cooking_status,$item_id);
					
					$transaction->OEX_KITCHEN_STATUS = implode( ", ", $cooking_status);
					$transaction->save();
    ...
    public function refreshOrdersList(Request $request)
    {
        ...
        return view('restaurant.partials.kitchen_show_orders', compact('orders', 'orders_for'));
    }
