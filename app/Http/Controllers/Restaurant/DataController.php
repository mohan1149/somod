        ...
        if (request()->ajax()) {
            ...
            if (!empty($location_id)) {
                ...
                if ($this->commonUtil->isModuleEnabled('tables')) {
                    $tables_enabled = true;
					$active_tables = Transaction::where('business_id', $business_id)
                            ->where('location_id', $location_id)->where('res_table_id','!=',null)->where('status','draft');
					                $transaction_id = $request->get('transaction_id', null);
                if (!empty($transaction_id)) {
					$active_tables = $active_tables->where('id','!=',$transaction_id);
					
				}
					
					$active_tables = $active_tables->pluck('res_table_id');
					
					
                    $tables = ResTable::where('business_id', $business_id)
                            ->where('location_id', $location_id)->whereNotIn('id',$active_tables)
                            ->pluck('name', 'id');
