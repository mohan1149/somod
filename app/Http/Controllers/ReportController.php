...
class ReportController extends Controller
{
    ...
    public function getTodayProfits(){
        ...
        return view('report.partials.profit_by_today');
    }

	public function getProfitLossPyamentMethods(Request $request){
		...
		foreach($payment_types as $key=>$payment){	
			...
			{
				$query = $query->where('t.location_id', $location_id);
			}
			$amount = $query->where('method',$key)->whereBetween('paid_on',[$start_date,$end_date])->sum('amount');
			
			if($amount>0)
			{
			$payments_report[$key]['name'] = $payment;
			$payments_report[$key]['amount'] = $amount;
			}
		}
		if(empty($payments_report))
			return null;
		return view('report.partials.profit_loss_details_payment_methods',compact('payments_report'));
	}
    /**
    ...
    public function getPurchaseSell(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $total_subscribers = DB::table('contacts')
                ->where('type','customer')
            ...
            if($location_id){
                $total_expenses = DB::table('transactions')
                    ...
                    ->whereBetween('created_at', [$start_date, $end_date])
                    ->sum('final_total');
                $total_paid_expenses = DB::table('transactions')
                    ->where('type','expense')
                    ->where('location_id',$location_id)
                    ->where('payment_status','paid')
                    ->whereBetween('created_at', [$start_date, $end_date])
                    ->sum('final_total');
                $total_unpaid_expenses = DB::table('transactions')
                    ->where('type','expense')
                    ->where('location_id',$location_id)
            ...
            $total_outside_orders = DB::table('tbl_outside_orders')
                ->whereBetween('order_date', [$start_date, $end_date])
                ->count();
            $total_outside_orders_completed = DB::table('tbl_outside_orders')
                ->where('order_status',3)
                ->whereBetween('order_date', [$start_date, $end_date])
                ->count();
            $total_outside_orders_value = DB::table('tbl_outside_orders')
            ...
            return [
                    ...
                    'total_unpaid_subscribers' => $total_unpaid_subscribers,
                    'total_expenses' => $total_expenses,
                    'total_paid_expenses' => $total_paid_expenses,
                    'total_unpaid_expenses' => $total_unpaid_expenses,
                    'total_outside_orders' => $total_outside_orders,
                    'total_outside_orders_completed'=> $total_outside_orders_completed,
                    'total_outside_orders_value' => $total_outside_orders_value
     ...
     * @return \Illuminate\Http\Response
     */
    public function getStockReport2(Request $request)
    {
        if (!auth()->user()->can('stock_report.view')) {
            abort(403, 'Unauthorized action.');
        }
	 ...
	 public function getProductStockReport(Request $request)
    {
        ...
        return view('report.product_stock_report')
            ->with(compact('categories', 'brands', 'units', 'business_locations', 'show_manufacturing_data'));
    }
	
	
	public function getProductStockReport2(Request $request)
    {
        if (!auth()->user()->can('stock_report.view')) {
            abort(403, 'Unauthorized action.');
        }
    ...
    public function getExpenseReportV2(){
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($expenses)
                ...
                ->rawColumns(['final_total', 'payment_status', 'payment_due'])
                ->make(true);
        }
        ...
        return view('report.expense_report_v2')
